...
void usage(const char *prog)
{
    fprintf(stderr,
            "Usage:\n"
            "  %s create <filename.flash> <size>\n"
            "  %s build <filename.flash> <host_dir> [size]\n"
            "  %s [--flash <filename.flash>] ls [-l] [path]\n"
            "  %s [--flash <filename.flash>] tree [path]\n"
            "  %s [--flash <filename.flash>] put <host_file> <xipfs_file>\n"
            "  %s [--flash <filename.flash>] put <xipfs_file>\n"
            "  %s [--flash <filename.flash>] get <xipfs_file> <host_file>\n"
            "  %s [--flash <filename.flash>] get <xipfs_file>\n"
            "  %s [--flash <filename.flash>] mkdir <xipfs_dir>\n"
            "  %s [--flash <filename.flash>] rm <xipfs_file>\n"
            "  %s [--flash <filename.flash>] rmdir <xipfs_dir>\n"
            "  %s [--flash <filename.flash>] mv <xipfs_src> <xipfs_dst>\n"
            "  %s test\n"
            "  %s test_deep\n"
            "\n"
            "Options:\n"
...
int open_image(app_ctx_t *ctx, const char *flash_path, bool writable)
{
    ...
    if (st.st_size <= 0 || ((size_t)st.st_size % XIPFS_NVM_PAGE_SIZE) != 0) {
        fprintf(stderr,
                "Error: invalid flash image size (%lld). Must be a positive multiple of %d.\n",
    ...
    xipfs_workstation_nvm_numof = ctx->image_size / XIPFS_NVM_PAGE_SIZE;

    memset(&ctx->mount, 0, sizeof(ctx->mount));
    ctx->mount.magic = XIPFS_MAGIC;
    ctx->mount.mount_path = "/";
    ctx->mount.page_num = xipfs_workstation_nvm_numof;
...
int create_image(const char *flash_path, const char *size_raw)
{
    ...
    xipfs_workstation_nvm_numof = image_size / XIPFS_NVM_PAGE_SIZE;

    memset(&mount, 0, sizeof(mount));
    mount.magic = XIPFS_MAGIC;
    mount.mount_path = "/";
    mount.page_num = xipfs_workstation_nvm_numof;
    ...
    printf("Created '%s' with %zu bytes (%zu pages of %d bytes).\n",
           flash_path, image_size, xipfs_workstation_nvm_numof, XIPFS_NVM_PAGE_SIZE);
